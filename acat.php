 ...
 <?php include 'include/navad.php';?>
  ...
  <div class="dark-wrapper">
    <div class="container inner2">
      <h3 class="section-title text-center">photos</h3>
      <p class="text-center">All Photos By Selected Catagories</p>
      

	  <?php

				include 'include/dmouse.php';
				//$mmid=$_SESSION['mmid'];
				//$ccat=$_SESSION['cat'];
				$b=$_GET['cat'];
				...
				//$sql="SELECT `catagoryid`,`filepath`,`mid` FROM `photos` WHERE `catagoryid`='$b'";
				$c="SELECT `catagoryid` FROM `photocatagory` WHERE `catagoryid`='$b'";
				$newsql="SELECT `models`.`mname`,`photos`.`filepath`,`photos`.`catagoryid` FROM `photos` INNER JOIN `models` ON `photos`.`mid`=`models`.`mid` WHERE `photos`.`catagoryid`='$b'";
		//var_dump($sql);
		//$res=mysqli_query($conn,$sql);
		$dis=mysqli_query($conn,$c);
		$newres=mysqli_query($conn,$newsql);
		//var_dump($res);
		while($diss=mysqli_fetch_assoc($dis))
	{
		echo "<h1 class='section-title text-center'>".$diss["catagoryid"]."</h1>";
	}		
	...
	*/
		...
		<a href='#'><img src='".$row["filepath"]."' alt=' 'width='1000px' height='400px'/></a>
            <div class='caption bottom-right'>
              ...
              <!--/.title--> <span class='main-title layer'>".$row["mname"]."</span>
            </div>
            <!--/.caption-->  </div>
			<p><p>
