        ...
        <li data-transition="fade"> <img src="style/images/art/slider-bg1.jpg"  alt="" data-bgposition="center top" data-bgfit="cover" data-bgrepeat="no-repeat" />
          ...
          <div class="tp-caption medium sfr" data-x="30" data-y="348" data-speed="900" data-start="1500" data-easing="Sine.easeOut">Welcome
		   ...
		   {header("Location:http:\adagency.php");}
			   else {echo $_SESSION['username'];}?><br />
         </div>
        </li>
...
<?php
				...
				$sql="SELECT `models`.`mname`,`photos`.`filepath`,`photos`.`catagoryid` FROM `models` INNER JOIN `photos` ON `photos`.`mid`=`models`.`mid`;";
		//var_dump($sql);
...
while($row=mysqli_fetch_assoc($res))
	...
	/*	echo"<div class='col-sm-4'>
		...
		echo"<div class='col-sm-4'>
         <div class='caption-overlay'>
		<a href='#'><img src='".$row["filepath"]."' alt='' width='5000px' height='250px' /></a> 
            <div class='caption bottom-right'><br>
              <div class='title'>
                <h3 class='main-title layer'>".$row["catagoryid"]."</h3>
				
              </div>
              <!--/.title --><span class='main-title layer'>".$row["mname"]."</span></p>
            </div>
            <!--/.caption --> 
			<p>
          </div>
